...
export function RewardsProfilePage() {
  ...
  if (!institutionId) {
    return (
      <Alert severity="warning">Your account is not linked to an institution yet.</Alert>
    );
  }
  ...
  return (
    <Box sx={{ maxWidth: 880 }}>
      ...
      <Typography sx={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 30, color: '#F3F6FB', mb: 1 }}>
        Rewards storefront
      </Typography>
      <Typography color="text.secondary" sx={{ mb: 3, maxWidth: 620 }}>
        Opt in to the Back2u rewards directory so members can discover you and spend their finder points at your
        venue. Your storefront is advertised for free across the app.
      </Typography>

      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: '1.5fr 1fr' }, gap: 3, alignItems: 'start' }}>
        <Box sx={PANEL}>
          {feedback && (
          ...
          <Stack spacing={2.5}>
            ...
            />
            <FormControlLabel
              control={<Switch checked={redeemable} onChange={(e) => setRedeemable(e.target.checked)} />}
              label="Accept point redemptions at our counter"
            />
            <TextField select label="Category" value={type} onChange={(e) => setType(e.target.value as InstitutionType)} fullWidth>
              {TYPES.map((t) => (
            ...
            <Box>
              ...
              <Stack direction="row" spacing={1.5} alignItems="center">
                ...
                <Button
                  ...
                  startIcon={uploadingLogo ? <CircularProgress size={16} /> : <PhotoCameraOutlinedIcon />}
                ...
                {logoUrl && !uploadingLogo && (
                  <Button color="inherit" onClick={() => setLogoUrl('')} sx={{ color: 'text.secondary' }}>
                    Remove
                  </Button>
                )}
              </Stack>
        ...
        <Box sx={PANEL}>
          <Typography sx={{ fontWeight: 700, color: '#F3F6FB', mb: 2 }}>Directory preview</Typography>
          <Box sx={{ borderRadius: 3, overflow: 'hidden', border: '1px solid rgba(255,255,255,0.08)' }}>
            <Box sx={{ height: 110, position: 'relative', overflow: 'hidden', display: 'grid', placeItems: 'center', background: logoUrl ? undefined : `linear-gradient(135deg, ${TEAL}, #0B3D38)` }}>
              {logoUrl ? (
                <Box component="img" src={logoUrl} alt="" sx={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
              ) : (
            ...
            <Box sx={{ p: 2 }}>
              <Typography sx={{ fontFamily: DISPLAY, fontWeight: 600, fontSize: 18, color: '#F3F6FB' }}>
                {inst?.name ?? 'Your venue'}
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>
                {description || 'Your short description will appear here.'}
              </Typography>
              <Typography variant="caption" sx={{ color: '#E0A106', fontWeight: 700, display: 'block', mt: 1 }}>
                {TYPES.find((t) => t.value === type)?.label} · 100 pts ≈ GHS {((100 * (Number(rate) || 0)) / 100).toFixed(2)}
              </Typography>
              {!listed && (
