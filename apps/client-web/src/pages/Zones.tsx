...
export function ZonesPage() {
  ...
  return (
    <Box maxWidth={820} mx="auto">
      ...
      <Stack spacing={2.5}>
        <SectionCard icon={<AddLocationAltOutlinedIcon />} title="Create a zone" desc="Draw a polygon on geojson.io and paste the coordinates.">
          <Stack spacing={2}>
            ...
            {create.isError && <Alert severity="error">Couldn’t save that zone — check the coordinates are valid JSON.</Alert>}
          </Stack>
        </SectionCard>
                    ...
                    <Stack direction="row" spacing={0.5} mt={0.75} flexWrap="wrap" useFlexGap>
                      {z.channels.map((ch) => (
                        <Chip
                          ...
                          size="small"
                          label={ch}
                          sx={{ height: 20, bgcolor: 'rgba(15,118,110,0.12)', color: TEAL, fontWeight: 600, textTransform: 'capitalize' }}
                        />
                      ))}
                    </Stack>
                  </Box>
