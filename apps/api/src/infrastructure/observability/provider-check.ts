 ...
 */
export function checkCriticalProviders(env: Env, logger: ILogger, reporter: IErrorReporter): void {
  const checks: ProviderCheck[] = [
    ...
    { key: 'ARKESEL_API_KEY', present: !!env.ARKESEL_API_KEY, critical: true },
    { key: 'ARKESEL_SENDER_ID', present: !!env.ARKESEL_SENDER_ID, critical: true },
    ...
    { key: 'PAYSTACK_SECRET_KEY', present: !!env.PAYSTACK_SECRET_KEY, critical: false },
  ...
  for (const m of missing) {
    if (isProd && m.critical) {
      logger.error(
        `Critical provider key missing: ${m.key} — feature will be degraded in production`,
        {
          key: m.key,
        },
      );
