...
export class Back2uClient {
  ...
  login(input: LoginInput) {
    return this.request<LoginResponse>('POST', '/v1/auth/login', input);
  }
  verifyMfaLogin(mfaToken: string, code: string) {
    return this.request<AuthResponse>('POST', '/v1/auth/mfa/verify', { mfaToken, code });
  }
  async changePassword(input: ChangePasswordInput) {
    // Send our own refresh token so the server spares this session from the
    // revoke-all that follows a password change.
    const refreshToken = (await this.getRefreshToken?.()) ?? undefined;
    return this.request<{ ok: true }>('POST', '/v1/auth/password/change', {
      ...input,
      refreshToken,
    });
  }
  refresh(refreshToken: string) {
    return this.request<AuthResponse>('POST', '/v1/auth/refresh', { refreshToken });
  }
  me() {
  ...
  updateProfile(input: UpdateProfileInput) {
    return this.request<UserDTO>('PATCH', '/v1/me', input);
  }
  setupMfa() {
    return this.request<MfaSetupResponse>('POST', '/v1/me/mfa/setup');
  }
  enableMfa(code: string) {
    return this.request<UserDTO>('POST', '/v1/me/mfa/enable', { code });
  }
  disableMfa(code: string) {
    return this.request<UserDTO>('POST', '/v1/me/mfa/disable', { code });
  }
  updateEmailPreferences(input: EmailPreferences) {
    return this.request<UserDTO>('PATCH', '/v1/me/preferences', input);
  }
  registerPushToken(token: string) {
    return this.request<{ ok: true }>('POST', '/v1/me/push-token', { token });
  }
  setLocale(locale: Locale) {
    return this.request<{ ok: true }>('POST', '/v1/me/locale', { locale });
  }
  redeemPoints(points: number) {
    return this.request<{ pointsBalance: number }>('POST', '/v1/me/redeem', { points });
  }
  exportMyData() {
    return this.request<unknown>('GET', '/v1/me/export');
  }
  describeImage(imageUrl: string) {
    return this.request<{ title: string; description: string; tags: string[] }>(
      'POST',
      '/v1/me/ai/describe-image',
      { imageUrl },
    );
  }
  ...
  // ---- Items ----
  autocompleteItems(q: string) {
    return this.request<AutocompleteResult>(
      'GET',
      `/v1/items/autocomplete?q=${encodeURIComponent(q)}`,
    );
  }
  ...
  createItem(input: CreateItemInput) {
    return this.request<ItemDTO>('POST', '/v1/items', input);
  }
  getItem(id: string) {
    return this.request<ItemDTO>('GET', `/v1/items/${encodeURIComponent(id)}`);
  }
  updateItem(id: string, input: UpdateItemInput) {
    return this.request<ItemDTO>('PATCH', `/v1/items/${encodeURIComponent(id)}`, input);
  }
  bumpItem(id: string) {
    return this.request<ItemDTO>('POST', `/v1/items/${encodeURIComponent(id)}/bump`);
  }
  ...
  // ---- Matches ----
  listMatchesForItem(itemId: string) {
    return this.request<MatchDTO[]>('GET', `/v1/items/${encodeURIComponent(itemId)}/matches`);
  }
  acceptMatch(matchId: string) {
    return this.request<MatchDTO>('POST', `/v1/matches/${encodeURIComponent(matchId)}/accept`);
  }
  rejectMatch(matchId: string) {
    return this.request<MatchDTO>('POST', `/v1/matches/${encodeURIComponent(matchId)}/reject`);
  }
  confirmItemReturn(matchId: string) {
    return this.request<MatchDTO>(
      'POST',
      `/v1/matches/${encodeURIComponent(matchId)}/confirm-return`,
    );
  }

  // ---- Chat ----
  listThreads() {
    return this.request<ChatThreadDTO[]>('GET', '/v1/chat/threads');
  }
  ...
  sendMessage(input: SendMessageInput) {
    return this.request<ChatMessageDTO>(
      'POST',
      `/v1/chat/threads/${encodeURIComponent(input.threadId)}/messages`,
      { body: input.body, images: input.images },
    );
  }
  markMessageRead(threadId: string, messageId: string) {
    return this.request<{ ok: true }>(
      'POST',
      `/v1/chat/threads/${encodeURIComponent(threadId)}/messages/${encodeURIComponent(messageId)}/read`,
    );
  }
  sendTyping(threadId: string, typing: boolean) {
    return this.request<{ ok: true }>(
      'POST',
      `/v1/chat/threads/${encodeURIComponent(threadId)}/typing`,
      { typing },
    );
  }
  ...
  releaseReward(id: string, finderId: string) {
    return this.request<RewardDTO>('POST', `/v1/rewards/${encodeURIComponent(id)}/release`, {
      finderId,
    });
  }
  ...
  // ---- Web Push ----
  getWebPushKey() {
    return this.request<{ vapidPublicKey: string | null }>('GET', '/v1/web-push/key');
  }
  ...
  // ---- QR Tags ----
  listMyTags() {
    return this.request<QrTagDTO[]>('GET', '/v1/tags/mine');
  }
  mintTags(count: number) {
    return this.request<QrTagDTO[]>('POST', '/v1/tags/mint', { count });
  }
  claimTag(code: string, itemLabel?: string) {
    return this.request<QrTagDTO>('POST', '/v1/tags/claim', { code, itemLabel });
  }
  getTagByCode(code: string) {
    return this.request<QrTagDTO>('GET', `/v1/tags/${encodeURIComponent(code)}`);
  }
  scanTag(code: string, finderMessage: string, finderEmail?: string) {
    return this.request<{ ownerName?: string; status: string }>(
      'POST',
      `/v1/tags/${encodeURIComponent(code)}/scan`,
      { finderMessage, finderEmail },
    );
  }
  sendTagHeartbeat(tagCode: string, lng: number, lat: number) {
    return this.request<{ received: true }>('POST', '/v1/tags/heartbeat', {
      tagCode,
      point: { lng, lat },
    });
  }
  markTagLost(code: string) {
    return this.request<QrTagDTO>('POST', `/v1/tags/${encodeURIComponent(code)}/lost`);
  }

  // ---- QR Tag Shop ----
  listTagProducts() {
    return this.request<QrTagProductDTO[]>('GET', '/v1/tags/products');
  }
  ...
  listMyTagOrders() {
    return this.request<QrTagOrderDTO[]>('GET', '/v1/tags/orders/my');
  }
  ...
  decideVerification(id: string, decision: 'approve' | 'reject', note?: string) {
    return this.request<OwnershipVerificationDTO>(
      'POST',
      `/v1/verifications/${encodeURIComponent(id)}/decide`,
      { decision, note },
    );
  }
  ...
  listMyCourierJobs() {
    return this.request<CourierJobDTO[]>('GET', '/v1/courier/jobs/my');
  }
  getCourierJob(id: string) {
    return this.request<CourierJobDTO>('GET', `/v1/courier/jobs/${encodeURIComponent(id)}`);
  }
  acceptCourierJob(id: string) {
    return this.request<CourierJobDTO>('POST', `/v1/courier/jobs/${encodeURIComponent(id)}/accept`);
  }
  transitionCourierJob(
    id: string,
    transition: 'pickup' | 'in_transit' | 'deliver' | 'cancel',
    code?: string,
  ) {
    return this.request<CourierJobDTO>(
      'POST',
      `/v1/courier/jobs/${encodeURIComponent(id)}/transition`,
      { transition, code },
    );
  }
  ...
  // ---- Vault ----
  listVault() {
    return this.request<VaultEntryDTO[]>('GET', '/v1/vault');
  }
  createVaultEntry(input: CreateVaultEntryInput) {
    return this.request<VaultEntryDTO>('POST', '/v1/vault', input);
  }
  deleteVaultEntry(id: string) {
    return this.request<{ deleted: true }>('DELETE', `/v1/vault/${encodeURIComponent(id)}`);
  }

  // ---- Zones ----
  listZones() {
    return this.request<ZoneSubscriptionDTO[]>('GET', '/v1/zones');
  }
  createZone(input: CreateZoneSubscriptionInput) {
    return this.request<ZoneSubscriptionDTO>('POST', '/v1/zones', input);
  }
  deleteZone(id: string) {
    return this.request<{ deleted: true }>('DELETE', `/v1/zones/${encodeURIComponent(id)}`);
  }

  // ---- Marketplace ----
  listMarketplace() {
    return this.request<MarketplaceListingDTO[]>('GET', '/v1/marketplace');
  }
  getMarketplaceListing(id: string) {
    return this.request<MarketplaceListingWithBidsDTO>(
      'GET',
      `/v1/marketplace/${encodeURIComponent(id)}`,
    );
  }
  createListing(input: {
    itemId: string;
  ...
  placeBid(input: PlaceBidInput) {
    return this.request<BidDTO>(
      'POST',
      `/v1/marketplace/${encodeURIComponent(input.listingId)}/bids`,
      { amount: input.amount },
    );
  }
  listMyBids() {
  ...
  closeMarketplaceListing(id: string) {
    return this.request<{ winnerId?: string; winningAmount: number }>(
      'POST',
      `/v1/marketplace/${encodeURIComponent(id)}/close`,
    );
  }

  // ---- Police ----
  generateStolenReport(itemId: string) {
    return this.request<PoliceCaseRefDTO>(
      'POST',
      `/v1/police/items/${encodeURIComponent(itemId)}/report`,
    );
  }
  filePoliceCase(id: string, caseNumber: string, station: string) {
    return this.request<PoliceCaseRefDTO>(
      'POST',
      `/v1/police/cases/${encodeURIComponent(id)}/file`,
      { caseNumber, station },
    );
  }
  ...
  // ---- Institutions ----
  listInstitutions() {
    return this.request<InstitutionDTO[]>('GET', '/v1/institutions');
  }
  getInstitution(id: string) {
    return this.request<InstitutionDTO>('GET', `/v1/institutions/${encodeURIComponent(id)}`);
  }
  ...
  getSubscriptionPlans() {
    return this.request<SubscriptionPlanDTO[]>('GET', '/v1/institutions/plans');
  }
  subscribeInstitution(id: string, tier: SubscriptionTier) {
    return this.request<InstitutionDTO>(
      'POST',
      `/v1/institutions/${encodeURIComponent(id)}/subscribe`,
      { tier },
    );
  }
  onboardInstitution(input: {
    ...
    return this.request<{ institution: InstitutionDTO; apiKey: string }>(
      'POST',
      '/v1/institutions',
      input,
    );
  ...
  decideInstitutionLead(id: string, decision: 'contacted' | 'approved' | 'rejected') {
    return this.request<InstitutionLeadDTO>(
      'POST',
      `/v1/institutions/leads/${encodeURIComponent(id)}/decide`,
      { decision },
    );
  }
  ...
  listInstitutionRedemptions(institutionId: string) {
    return this.request<RedemptionDTO[]>(
      'GET',
      `/v1/redemptions/institution/${encodeURIComponent(institutionId)}`,
    );
  }
  ...
  getShareCard(itemId: string) {
    return this.request<SocialShareCardDTO>(
      'GET',
      `/v1/share/items/${encodeURIComponent(itemId)}/share-card`,
    );
  }

  // ---- Admin ----
  getAdminStats() {
    return this.request<AdminStatsDTO>('GET', '/v1/admin/stats');
  }

  // ---- Partner ----
  getPartnerStats() {
    return this.request<PartnerStatsDTO>('GET', '/v1/partner/stats');
  }

  // ---- Partner API (public, API-key auth) ----
  listPartnerApiItems(
    query: ItemListQuery & { dateFrom?: string; dateTo?: string } = {},
    apiKey: string,
  ) {
    const qs = new URLSearchParams();
    if (query.kind) qs.set('kind', query.kind);
    ...
    if (query.page) qs.set('page', String(query.page));
    if (query.pageSize) qs.set('pageSize', String(query.pageSize));
    return this.request<Paginated<ItemDTO>>(
      'GET',
      `/partner/v1/items?${qs.toString()}`,
      undefined,
      {
  ...
  updatePartnerApiItemStatus(id: string, status: string, apiKey: string) {
    return this.request<ItemDTO>(
      'PATCH',
      `/partner/v1/items/${encodeURIComponent(id)}/status`,
      { status },
      {
        headers: { 'X-API-Key': apiKey },
      },
  ...
  createPartnerApiKey(institutionId: string, name: string) {
    return this.request<CreatedPartnerApiKeyDTO>('POST', '/v1/admin/partner-api-keys', {
      institutionId,
      name,
    });
  }
  ...
  revokePartnerApiKey(id: string) {
    return this.request<{ deleted: true }>(
      'DELETE',
      `/v1/admin/partner-api-keys/${encodeURIComponent(id)}`,
    );
  }
  ...
  updateUserStatus(id: string, status: string, reason?: string) {
    return this.request<UserDTO>('PATCH', `/v1/users/${encodeURIComponent(id)}/status`, {
      status,
      reason,
    });
  }
  ...
  listTrustedFinderApplications(status?: 'pending' | 'approved' | 'rejected') {
    ...
    return this.request<TrustedFinderApplicationDTO[]>(
      'GET',
      `/v1/trusted-finder/applications${qs}`,
    );
  }
  decideTrustedFinderApplication(id: string, decision: 'approved' | 'rejected', reason?: string) {
    return this.request<{ application: TrustedFinderApplicationDTO; user?: UserDTO }>(
      'POST',
      `/v1/trusted-finder/applications/${encodeURIComponent(id)}/decide`,
      { decision, reason },
    );
  }
  ...
  requestPhoneOtp(phone: string) {
    return this.request<{ sent: true }>('POST', '/v1/auth/phone/request-otp', { phone });
  }
  verifyPhoneOtp(phone: string, code: string) {
    return this.request<{ verified: true }>('POST', '/v1/auth/phone/verify', { phone, code });
  }
  requestEmailVerification() {
    return this.request<{ sent: true }>('POST', '/v1/auth/email/request-verification');
  }
  confirmEmailVerification(code: string) {
    return this.request<{ verified: true }>('POST', '/v1/auth/email/confirm', { code });
  }
  requestPasswordReset(email: string) {
    return this.request<{ sent: true }>('POST', '/v1/auth/password/request-reset', { email });
  }
  confirmPasswordReset(token: string, newPassword: string) {
    return this.request<{ ok: true }>('POST', '/v1/auth/password/confirm', { token, newPassword });
  }
  logout(refreshToken: string) {
    return this.request<{ ok: true }>('POST', '/v1/auth/logout', { refreshToken });
  }

  // ---- Account ----
  exportAccount() {
    return this.request<unknown>('GET', '/v1/account/export');
  }
  deleteAccount() {
  ...
  createWebhook(input: CreateWebhookInput) {
    return this.request<WebhookDTO>('POST', '/v1/webhooks', input);
  }
  listWebhooks() {
    return this.request<WebhookDTO[]>('GET', '/v1/webhooks');
  }
  updateWebhook(id: string, input: UpdateWebhookInput) {
    return this.request<WebhookDTO>('PATCH', `/v1/webhooks/${encodeURIComponent(id)}`, input);
  }
  deleteWebhook(id: string) {
    return this.request<{ deleted: true }>('DELETE', `/v1/webhooks/${encodeURIComponent(id)}`);
  }

  // ---- Notifications ----
  listNotifications(limit = 50) {
    return this.request<NotificationDTO[]>('GET', `/v1/notifications?limit=${limit}`);
  }
  markNotificationRead(id: string) {
    return this.request<{ ok: true }>('POST', `/v1/notifications/${encodeURIComponent(id)}/read`);
  }
  markAllNotificationsRead() {
    return this.request<{ ok: true }>('POST', '/v1/notifications/read-all');
  }
  getUnreadNotificationCount() {
    return this.request<{ count: number }>('GET', '/v1/notifications/unread-count');
  }

  // ---- Bookmarks ----
  bookmarkItem(itemId: string) {
    return this.request<BookmarkDTO>('POST', '/v1/bookmarks', { itemId });
  }
  unbookmarkItem(itemId: string) {
    return this.request<{ deleted: true }>('DELETE', `/v1/bookmarks/${encodeURIComponent(itemId)}`);
  }
  listBookmarks() {
    return this.request<BookmarkDTO[]>('GET', '/v1/bookmarks');
  }
  ...
  getMyReviewForMatch(matchId: string) {
    return this.request<ReviewDTO | null>(
      'GET',
      `/v1/reviews/match/${encodeURIComponent(matchId)}/mine`,
    );
  }

  // ---- Safety ----
  listBlocks() {
    return this.request<{ blockedId: string; createdAt: string }[]>('GET', '/v1/safety/blocks');
  }
  blockUser(blockedId: string) {
    return this.request<{ ok: true }>('POST', '/v1/safety/blocks', { blockedId });
  }
  unblockUser(blockedId: string) {
    return this.request<{ ok: true }>(
      'DELETE',
      `/v1/safety/blocks/${encodeURIComponent(blockedId)}`,
    );
  }
  fileReport(input: {
  ...
  listOpenReports() {
    return this.request<import('@back2u/shared-types').SafetyReportDTO[]>(
      'GET',
      '/v1/safety/reports',
    );
  }
  decideReport(id: string, decision: 'action' | 'dismiss' | 'resolved', note?: string) {
    return this.request<{ ok: true }>(
      'POST',
      `/v1/safety/reports/${encodeURIComponent(id)}/decide`,
      { decision, note },
    );
  }
  ...
  listModerationQueue(filter: { type?: string; status?: string } = {}) {
    ...
    return this.request<import('@back2u/shared-types').ModerationQueueItemDTO[]>(
      'GET',
      `/v1/moderation/queue?${qs.toString()}`,
    );
  }
  reviewModerationItem(id: string, decision: 'approve' | 'remove') {
    return this.request<{ ok: true }>('POST', `/v1/moderation/${encodeURIComponent(id)}/review`, {
      decision,
    });
  }
  ...
  // ---- Feature Flags ----
  listFeatureFlags() {
    return this.request<FeatureFlagWithStatusDTO[]>('GET', '/v1/features');
  }
  isFeatureEnabled(key: string) {
    return this.request<{ enabled: boolean }>(
      'GET',
      `/v1/features/${encodeURIComponent(key)}/enabled`,
    ).then((r) => r.enabled);
  }
  toggleFeatureFlag(key: string) {
    return this.request<FeatureFlagDTO>('POST', `/v1/features/${encodeURIComponent(key)}/toggle`);
  }
  updateFeatureRollout(key: string, input: UpdateRolloutInput) {
    return this.request<FeatureFlagDTO>(
      'POST',
